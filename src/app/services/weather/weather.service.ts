...
export class WeatherService {
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=f267b7f7342cdaf99dd416526e34ba0b`)
      .subscribe((data) => {
  ...
  getWeatherState(city: string): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=f267b7f7342cdaf99dd416526e34ba0b`)
      .subscribe((data) => {
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=f267b7f7342cdaf99dd416526e34ba0b`)
      .subscribe((weather: any) => {
